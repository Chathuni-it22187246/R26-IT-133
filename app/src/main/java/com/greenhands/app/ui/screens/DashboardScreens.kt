...
fun DashboardModule.navigationRoute(): String = when {
    isHeat -> Routes.CROPS
    isSensor -> Routes.SENSOR_SETUP
    else -> Routes.comingSoon(id)
}
...
fun dashboardModules(): List<DashboardModule> = listOf(
    DashboardModule(
        ...
        stringResource(R.string.module_sensor_title),
        stringResource(R.string.module_sensor_body),
        isSensor = true
    ),
...
fun DashboardScreen(
    ...
    environment: GreenhouseEnvironmentSnapshot = PreviewEnvironment.snapshot,
        ...
        InfoCard(modifier = Modifier.testTag("dashboard_environment")) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Column {
                    ...
                    Text(
                        environment.temperatureC?.let { String.format(java.util.Locale.US, "%.1f°C", it) } ?: "—",
                        style = GhType.metric,
                        color = ClimateTeal,
                ...
                Column {
                    ...
                    Text(
                        environment.relativeHumidityPercent?.let { String.format(java.util.Locale.US, "%.0f%%", it) } ?: "—",
                        style = GhType.metric,
                        color = ClimateTeal,
                ...
                Column(horizontalAlignment = Alignment.End) {
                    StatusChip(
                        stringResource(R.string.dashboard_sample_values),
...
private fun DashboardModuleCard(
    ...
    val status = when {
        ...
        module.isSensor -> stringResource(R.string.module_sensor_status)
